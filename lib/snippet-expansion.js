...
export class SnippetExpansion{
  ...
  prepareArguments(language_name, snippet_text, put_cursor){
    var args = [];
  ...
  expand(put_cursor = true){
    ...
    this.execFile(this.homotopy_command, this.prepareArguments(language_name, snippet_text, put_cursor),
      (error, stdout, stderr) =>{
        if(error){
